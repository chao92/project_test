...
#
###################################
def gen_syn_shape(nb_shapes=80, width_basis=300, feature_generator=None, shape_type="house", add_random_edges=0, m=5, seed=0):
    """ Synthetic Graph #1:
    ...
    if shape_type == "house":
        ...
        list_shapes = [[shape_type, 2]] * nb_shapes
    elif shape_type == "cycle":
        list_shapes = [[shape_type, 6]] * nb_shapes
    elif shape_type == "clique":
        list_shapes = [[shape_type, 4]] * nb_shapes
    elif shape_type == "fan":
        list_shapes = [[shape_type, 3]] * nb_shapes
    ...
    # print(" m is ", m , "seed is ", seed)
    G, role_id, _ = synthetic_structsim.build_graph(
        width_basis, basis_type, list_shapes, start=0, add_random_edges=add_random_edges, m=m, seed=seed
    )
    G = perturb([G], 0.01)[0]
    ...
    for i in range(len(G.nodes())):
        print(" feature generator is", G.nodes[i]["feat"])

    name = "shape_type" + shape_type + "_baseNode_" + str(width_basis) + "_baseNum_" + str(nb_shapes)
...
#
###################################
def gen_super_graph(start, width, m):
    graph, roles = synthetic_structsim.ba(start, width, role_start=0, m=m, seed=0)
    print(" graph", graph)
    print("roles", roles )
    return graph

def gen_syn1(nb_shapes=80, width_basis=300, feature_generator=None, add_random_edges=0, m=5, seed=0):
    """ Synthetic Graph #1:
    ...
    name = "baseType_" + basis_type + "_baseNode_" + str(width_basis) + "_baseNum_" + str(nb_shapes)
    return G, role_id, name
