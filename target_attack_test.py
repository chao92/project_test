...
from attack.DAGPA import GNNAttack
...
def build_args():
    ...
    parser.add_argument('--random', type=str2bool, default=False, help='random mask')
...
if __name__ == '__main__':
    ...
    baseline_model_ckpt_path = osp.join('checkpoints', data_name, 'GCN_2l', 'GCN_2l_best.ckpt')
    ...
    if attack_graph:
        ...
        attacker = GNNAttack(model, new_node_num=added_node_num, epochs=args.attack_epochs, lr=0.005, attack_graph=attack_graph,
                         mask_features=args.feature_attack, mask_structure=args.structure_attack, random=args.random, random_feature=args.random_feature, args=args)
    else:
    ...
    if not args.attack_graph:
        ...
        if pred_class == args.desired_class:
            ...
            success = False
        tmp_list.append([args.node_idx, pred_class])
    ...
    path = None
    if not args.feature_attack:
        path = f'results/{data_name}/attack_with_structure/added_node_{added_node_num}/train_percent_{args.train_percent}'
    else:
        path = f'results/{data_name}/attack_with_structure_feature/added_node_{added_node_num}/train_percent_{args.train_percent}'
    if not osp.isdir(path):
        os.makedirs(path)

    if args.random == True:
        file = f'{path}/sparsity_percent_{args.sparsity}_random_res.csv'
    else:
        if args.random_feature:
            file = f'{path}/structure_sparsity_{args.sparsity}_feature_sparsity_{args.feat_sparsity}_random_res.csv'
        else:
            file = f'{path}/structure_sparsity_{args.sparsity}_feature_sparsity_{args.feat_sparsity}_res.csv'
