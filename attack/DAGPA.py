...
class GNNAttack(AttackBase):
    ...
    def __init__(self, model, new_node_num=0, epochs=100, lr=0.01, attack_graph=True, mask_features=False, mask_structure=True, random=False, random_feature=False, \
                 args=None):
        if args:
        ...
        super(GNNAttack, self).__init__(model, new_node_num, epochs, lr, attack_graph, mask_features, mask_structure, random, random_feature)
    ...
    def forward(self, x, edge_index, y, **kwargs):
        ...
        if not self.attack_graph:
            ...
            print(" target attack, node ix is", node_idx)
        ...
        if self.random_mask:
            print(" --------- random mask attack ---------")
            f_mask, e_mask = self.node_feat_mask.data.clone(), self.edge_mask.data.clone()
        else:
            # test feature sparsity, if feature is also random, then directly skip learning process
        ...
        tmp_feature_mask, tmp_edge_mask,  structure_sp, feature_sp = self.control_sparsity(f_mask, e_mask, fix_sparsity = kwargs.get('fix_sparsity'), \
                                                                                           sparsity=kwargs.get('sparsity'), feat_sparsity=kwargs.get('feat_sparsity'))
